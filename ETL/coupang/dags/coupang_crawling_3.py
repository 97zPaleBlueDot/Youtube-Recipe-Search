...
def ingredientsDetailResult(response, ingredient_name, ingredient_result):
    ...
    for li in lis:
        ...
        if li.find("span", class_=rank_class_name) is not None:
            ...
            origial_price = (
                int(li.find("del", class_="base-price").text.replace(",", ""))
                if li.find("del", class_="base-price")
                else None
            )
            ...
            discount_rate = (
                li.find("span", class_="instant-discount-rate").text
                if li.find("span", class_="instant-discount-rate")
                else None
            )
            ...
            review_count = (
                int(li.find("span", class_="rating-total-count").text[1:-1])
                if li.find("span", class_="rating-total-count")
                else None
            )
