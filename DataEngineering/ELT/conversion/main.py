...
if __name__ == "__main__":
    ...
    for i in range(len(ingredient_list)):
        ...
        if ingredient_volume:  # 1. 정규식 표현으로 처리가 가능한 경우.
            db_connector.write_to_ingredient(
            ...
            ingredient_volume, ingredient_unit = preprocessor.split_volume_unit(
                input_string=ingredient_vague
            )

            if ingredient_volume and ingredient_unit:
                converted_volume, standard_unit = (
                    db_connector.get_converted_volume_and_standard_unit(
                ...
                if converted_volume and standard_unit:
                    ...
                    temp_ingredient_volume = preprocessor.extract_grams_by_regex(
                        input_string=temp_vague
                    )

                    if temp_ingredient_volume:  # 정규식 표현으로 1차 확인.
                        db_connector.write_to_ingredient(
                            ingredient_id=ingredient_id,
                            ingredient_volume=temp_ingredient_volume,
                            ingredient_unit="g",
                        )
                        continue

                    else:
                        converted_gram = db_connector.get_converted_gram(
                            ingredient_name=ingredient_name, unit_name=standard_unit
                        )
                        db_connector.write_to_ingredient(
                            ingredient_id=ingredient_id,
                            ingredient_volume=total_volume * converted_gram,
                            ingredient_unit="g",
                        )
                        continue
                    ...
                    converted_gram = db_connector.get_converted_gram(
                        ingredient_name=ingredient_name, unit_name=ingredient_unit
                    )
                    db_connector.write_to_ingredient(
                        ingredient_id=ingredient_id,
                        ingredient_volume=ingredient_volume * converted_gram,
                        ingredient_unit="g",
                    )
                    continue
            else:
